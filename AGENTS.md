...
`octo-pi` is a Bun/TypeScript Oh My Pi extension that adds multi-model debate and brainstorm workflows.

The extension registers:
- `/debate` and `/brainstorm` commands in `src/main.ts`
- `octopus_multi_model_round` and `octopus_next_step` tools in `src/main.ts`

Core modules:
- `src/arguments.ts` parses command arguments and `--models` selectors.
- `src/model-selection.ts` builds and renders the interactive model multi-select UI.
- `src/prompts.ts` builds the orchestrator prompt contract.
- `src/subagents.ts` runs participant model sessions concurrently and returns per-model results.
...
- Do not add fallback/mock participant behavior. `runMultiModelRound` should report real model errors in `RoundResult.error`.
